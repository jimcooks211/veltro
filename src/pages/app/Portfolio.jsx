...
══════════════════════════════════════════════════════════════ */
const WALLET = { balance: 12_450.38, reserved: 1_840.00, currency: 'USD' }

const RAW_HOLDINGS = [
  { stock_id:'s1', quantity:14,  avg_cost:168.42, total_invested:2_357.88, realised_pnl:0,      first_bought_at:'Jan 15, 2024',
  ...
  { stock_id:'s2', quantity:22,  avg_cost:108.30, total_invested:2_382.60, realised_pnl:362.00, first_bought_at:'Mar 2, 2024',
    symbol:'NVDA', name:'NVIDIA Corp.',       exchange:'NASDAQ', sector:'Technology', industry:'Semiconductors',
  ...
  { stock_id:'s3', quantity:7.5, avg_cost:385.00, total_invested:2_887.50, realised_pnl:188.50, first_bought_at:'Nov 8, 2023',
    symbol:'MSFT', name:'Microsoft Corp.',    exchange:'NASDAQ', sector:'Technology', industry:'Software',
  ...
  { stock_id:'s4', quantity:9,   avg_cost:220.10, total_invested:1_980.90, realised_pnl:75.60,  first_bought_at:'Feb 20, 2024',
    symbol:'JPM',  name:'JPMorgan Chase',     exchange:'NYSE',   sector:'Financials', industry:'Diversified Banks',
  ...
  { stock_id:'s5', quantity:4,   avg_cost:510.00, total_invested:2_040.00, realised_pnl:142.00, first_bought_at:'Dec 3, 2023',
    symbol:'META', name:'Meta Platforms',     exchange:'NASDAQ', sector:'Technology', industry:'Interactive Media',
  ...
  { stock_id:'s6', quantity:16,  avg_cost:115.20, total_invested:1_843.20, realised_pnl:-76.80, first_bought_at:'Apr 5, 2024',
    symbol:'XOM',  name:'ExxonMobil Corp.',   exchange:'NYSE',   sector:'Energy',      industry:'Oil & Gas',
  ...
  { stock_id:'s7', quantity:11,  avg_cost:163.40, total_invested:1_797.40, realised_pnl:-68.20, first_bought_at:'Jun 11, 2024',
  ...
  { stock_id:'s8', quantity:20,  avg_cost:91.50,  total_invested:1_830.00, realised_pnl:134.00, first_bought_at:'Aug 22, 2024',
    symbol:'WMT',  name:'Walmart Inc.',       exchange:'NYSE',   sector:'Consumer',    industry:'Hypermarkets',
...
function PieLbl({ cx, cy, midAngle, innerRadius, outerRadius, pct }) {
  ...
  return (
    <text
      ...
      style={{ fontSize:9, fontWeight:800, fontFamily:'DM Mono,monospace' }}>
      {pct.toFixed(0)}%
    </text>
  )
}
...
function SortBtn({ label, field, sk, sd, onSort }) {
  ...
  return (
    <button className={`pf-th-btn${on ? ' active' : ''}`} onClick={() => onSort(field)}>
      {label}
      {on
        ? (sd === 'desc' ? <CaretDown size={8} weight="bold"/> : <CaretUp size={8} weight="bold"/>)
...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    ]).then(([summary, portfolio, walletData]) => {
      setLiveSummary(summary)
      setLiveHoldings(portfolio.holdings || [])
      const usd = (walletData.balances || []).find(b => b.currency === 'USD')
      setLiveWallet({
        balance:  Number(usd?.balance  || 0),
        reserved: Number(usd?.reserved || 0),
      })
    }).catch(() => {}).finally(() => setLoading(false))
  }, [])

  const sorted = useMemo(() => {
    const positions = liveHoldings.map(h => {
      const last_price     = Number(h.avg_cost || 0)
      const market_value   = +(Number(h.quantity) * last_price).toFixed(2)
      const unrealised_pnl = +(market_value - Number(h.total_invested)).toFixed(2)
      const unrealised_pct = Number(h.total_invested) > 0
      ...
      return {
        ...
        volume: 0, market_cap: 0, pe_ratio: null,
        week_52_high: last_price * 1.2, week_52_low: last_price * 0.8,
        color: '#1A56FF', vs_52wk_high: 0, vs_52wk_low: 0,
      }
    })
  ...
  const UNREAL_PNL     = +(TOTAL_VALUE - TOTAL_INVESTED).toFixed(2)
  const REAL_PNL       = liveSummary ? Number(liveSummary.total_realised_pnl || 0) : 0
  const DAY_PNL        = 0
  const DAY_PCT        = 0
  const TOTAL_RETURN   = TOTAL_INVESTED > 0 ? +((UNREAL_PNL / TOTAL_INVESTED) * 100).toFixed(2) : 0
  ...
  const displayInvested    = TOTAL_INVESTED
  const displayRealisedPnl = REAL_PNL

  /* Empty NAV chart points for new users */
  const NAV_CHART = TOTAL_VALUE > 0 ? (() => {
    const labels = ['Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec','Jan','Feb','Mar']
    let v = TOTAL_INVESTED * 0.91
    return labels.map((m, i) => {
      v += (Math.random() - 0.40) * v * 0.048
  ...
  const hMin = NAV_CHART.length > 0 ? Math.min(...NAV_CHART.map(d => d.v)) * 0.97 : 0
  const hMax = NAV_CHART.length > 0 ? Math.max(...NAV_CHART.map(d => d.v)) * 1.015 : 1

  /* Sector breakdown from live holdings */
  const SECTORS = Object.entries(
    sorted.reduce((acc, p) => { acc[p.sector] = (acc[p.sector] || 0) + p.market_value; return acc }, {})
  ).map(([s, v]) => ({
    sector: s, value: v,
    pct: TOTAL_VALUE > 0 ? +((v / TOTAL_VALUE) * 100).toFixed(1) : 0,
    color: { Technology:'#1A56FF', Financials:'#C9A84C', Healthcare:'#00E676', Energy:'#FF8C42', Consumer:'#FFB800' }[s] || '#627EEA'
  })).sort((a, b) => b.value - a.value)
    ...
    {loading && (
      ...
      <div className="pf-hdr">
        ...
        <nav className="pf-bc">
          <span>Dashboard</span>
          <CaretRight size={9}/>
          <span className="pf-bc-c">Portfolio</span>
        </nav>
      </div>
      ...
      <div className="pf-hero">
        ...
        <div className="pf-hero-grid">
          {[
            ...
            { label:'Total Invested',    v: H(`$${f2(displayInvested)}`),                                          c:'var(--vlt-text-secondary)' },
            { label:'Unrealised P&L',    v: H(`${sgn(UNREAL_PNL)}$${f2(Math.abs(UNREAL_PNL))}`),                  c: UNREAL_PNL  >= 0 ? '#00C076' : '#FF3D57' },
            { label:'Realised P&L',      v: H(`${sgn(displayRealisedPnl)}$${f2(Math.abs(displayRealisedPnl))}`),  c: displayRealisedPnl >= 0 ? '#00C076' : '#FF3D57' },
            { label:'Total Return',      v: H(`${sgn(TOTAL_RETURN)}${Math.abs(TOTAL_RETURN).toFixed(2)}%`),        c: TOTAL_RETURN >= 0 ? '#00C076' : '#FF3D57' },
            { label:'Cash Available',    v: H(`$${f2(liveWallet.balance)}`),                                        c:'#FFB800' },
            { label:'Reserved (Orders)', v: H(`$${f2(liveWallet.reserved)}`),                                      c:'rgba(255,255,255,.45)' },
            { label:'Open Positions',    v: String(sorted.length),                                                  c:'var(--vlt-text-primary)' },
          ].map(s => (
      ...
      <div className="pf-charts">
        ...
        <div className="pf-card pf-nav-card">
          <div className="pf-card-top">
            ...
            <span
              className="pf-return-badge"
              style={{
                color:       navUp ? '#00C076' : '#FF3D57',
                background:  navUp ? 'rgba(0,192,118,.1)' : 'rgba(255,61,87,.1)',
              }}>
        ...
        <div className="pf-card pf-sector-card">
          <div className="pf-card-top">
            <span className="pf-card-title">
              <ChartPieSlice size={13} weight="duotone"/>
              Sector Allocation
            </span>
          </div>
        ...
        <div className="pf-tabs">
          {[
            ...
            <button
              ...
              onClick={() => setTab(t.id)}>
              {t.label}
              {t.count != null && <span className="pf-tab-badge">{t.count}</span>}
            </button>
          ))}
        </div>
        ...
        {tab === 'holdings' && (
          <div className="pf-tbl-wrap">
              ...
              {sorted.map((p, i) => {
                ...
                return (
                  <div
                    ...
                    {/* Shares */}
                    <div className="pf-cell-num">
                      {p.quantity % 1 === 0 ? p.quantity : p.quantity.toFixed(3)}
                    </div>
                    ...
                    {/* P/E ratio */}
                    <div className="pf-cell-num dim">
                      {p.pe_ratio?.toFixed(1) ?? 'N/A'}
                    </div>
                    ...
                    <div className="pf-cell-range">
                      <div className="pf-range-track">
                        <div
                          ...
                          style={{ width:`${Math.max(0, Math.min(100, rangePct))}%`, background:p.color }}/>
                        <div
                          className="pf-range-knob"
                          style={{ left:`${Math.max(0, Math.min(100, rangePct))}%`, background:p.color }}/>
                      </div>
                    </div>
            ...
            <div className="pf-tfoot">
              ...
              <span className={`pf-tfoot-pnl ${UNREAL_PNL >= 0 ? 'g' : 'r'}`}>
                {H(`${sgn(UNREAL_PNL)}$${f2(Math.abs(UNREAL_PNL))}`)}
              </span>
              <span className={`pf-tfoot-ret ${TOTAL_RETURN >= 0 ? 'g' : 'r'}`}>
                {sgn(TOTAL_RETURN)}{Math.abs(TOTAL_RETURN).toFixed(2)}%
              </span>
              <span/><span/><span/><span/>
            </div>
          </div>
                ...
                <div key={i} className="pf-act-row">
                  ...
                  <div className="pf-act-mid">
                    ...
                    <span className="pf-act-detail">
                      {t.side === 'buy' ? 'Bought' : 'Sold'} {t.qty}{' '}
                      {t.qty === 1 ? 'share' : 'shares'} @ ${f2(t.price)}
                    </span>
                  </div>
